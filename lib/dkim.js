      ...
      for (const recordSegments of res) {
        ...
        if (this.dns_fields.g) {
          if (this.dns_fields.g !== '*') {
            ...
            this.debug(
              `${this.identity}: matching ${this.dns_fields.g} against i=${
                this.fields.i
              } regexp=${reg.toString()}`
            )
...
class DKIMSignStream extends Stream {
  ...
  write(buf) {
    ...
    // Create DKIM header
    let dkim_header = `v=1; a=rsa-sha256; c=relaxed/simple; d=${
      this.domain_name
    }; s=${this.selector}; h=${headers.join(':')}; bh=${bodyhash}; b=`
          ...
          if (!this.header_idx['dkim-signature']) {
            ...
            for (const dkimHeader of dkim_headers) {
              this.dkim_objects.push(
                new DKIMObject(dkimHeader, this.header_idx, callback, this.opts)
              )
            }
            if (this.pending === 0) {
              process.nextTick(() => {
